...
|   通信协议   |             基本语法              |                           功能&bug                           |
| :----------: | :-------------------------------: | :----------------------------------------------------------: |
|   注册协议   |  【register 用户名 密码 用户ID】  |                    用户ID目前需要自己指定                    |
|   登录协议   | 【login 用户名 密码 用户ID 邮箱】 |        用户名不需要匹配，但密码和ID匹配；邮箱可选择写        |
| 断线重连协议 |  【reconnect 用户ID 令牌token】   | 令牌是最近一次登录（存在挤号，后来居上），服务器返回的token值 |
| 进入游戏协议 |      【enter_scene 场景ID】       |         场景ID可选择写（1001 1002）；默认会随机进入          |
| 退出游戏协议 |      【leave_scene 场景ID】       |                        场景ID可选择写                        |
|   工作指令   |             【work】              |                            金币+1                            |
|   查看指令   |             【view】              |                         显示玩家信息                         |
